  ...
  module Model
    def acts_as_content_highlightable_on(column_name)
      if not self.column_names.include? column_name.to_s
        raise ArgumentError, "acts_as_content_highlightable_on: Invalid attribute"
      end
      ...
      class_eval %{
        ...
        def name_html_nodes
          self.#{column_name.to_s} = ActsAsContentHighlightable::HtmlNodeParser.new(self.#{column_name.to_s}).assign_unique_node_identifiers(Solution.unique_node_attribute_name).body_content
        end
      }
    end
  end
